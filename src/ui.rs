use std::{
    fmt::Write,
    process::exit,
    time::{Duration, Instant, SystemTime, UNIX_EPOCH}, io::{stdout},
};
...
pub fn print_ui(oxide: &mut WPOxideRuntime, start_time: Instant, framerate: u64) -> Result<(), std::io::Error> {
    ...
    match oxide.ui_state.menu {
        0 => execute!(stdout(), Print(access_points_pane(oxide, winsize, start_time, framerate))),
        1 => execute!(stdout(), Print(clients_pane(oxide, winsize, start_time, framerate))),
        2 => execute!(stdout(), Print(handshakes_pane(oxide, winsize, start_time, framerate))),
        3 => execute!(stdout(), Print(messages_pane(oxide, winsize, start_time, framerate))),
        _ => Ok({}),
    }
}
...
pub fn access_points_pane(oxide: &mut WPOxideRuntime, winsize: (u16, u16), start_time: Instant, framerate: u64) -> String {
    ...
    match oxide.ui_state.ap_sort {
        ...
        1 => access_points.sort_by(|a, b| b.1.last_signal_strength.value.cmp(&a.1.last_signal_strength.value)),
    ...
    write!(output, "{:─>width$}", "").ok();
    write!(output, "{:^width$}", "quit: [q] | sort APs: [a] | sort clients: [c] | reverse sort: [r] | change tab: [←]/[→]").ok();
